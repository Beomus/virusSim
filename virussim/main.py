...
class Virus:
    def __init__(self, params) -> None:
        ...
        self.mild = {
            i: {"thetas": [], "rs": []} for i in range(self.mild_fast, 365)
        }
        self.severe = {
            "recovery": {
                i: {"thetas": [], "rs": []}
                for i in range(self.severe_fast, 365)
            },
            "death": {
                i: {"thetas": [], "rs": []}
                for i in range(self.death_fast, 365)
            },
        }
    ...
    def one_by_one(self, i, thetas, rs, color) -> None:
        self.axes.scatter(thetas[i], rs[i], s=5, color=color)
        if i == len(thetas) - 1:
            self.anim2.event_source.stop()
            self.anim.event_source.start()

    def chunks(self, a_list, n) -> List:
        for i in range(0, len(a_list), n):
            yield a_list[i : i + n]

    def assign_symptoms(self) -> None:
        ...
        percent_severe_recover = 1 - (self.fatality_rate / self.percent_severe)
        num_severe_recovery = round(percent_severe_recover * num_severe)
    ...
    def update_status(self) -> None:
        ...
        if self.day >= self.severe_fast:
            recovery_thetas = self.severe["recovery"][self.day]["thetas"]
            recovery_rs = self.severe["recovery"][self.day]["rs"]
            self.axes.scatter(recovery_thetas, recovery_rs, s=5, color=GREEN)
            self.num_recovered += len(recovery_thetas)
            self.num_currently_infected -= len(recovery_thetas)
